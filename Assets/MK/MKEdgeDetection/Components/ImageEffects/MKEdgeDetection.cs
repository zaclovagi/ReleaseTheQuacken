...
/* Version: 1.1.69  */
/* Exported on: 13.11.2025 00:24:57 */
...
namespace MK.EdgeDetection.ImageEffectsComponents
{
	...
	public sealed class MKEdgeDetection : UnityEngine.MonoBehaviour, MK.EdgeDetection.ImageEffectsComponents.IParameters, MK.EdgeDetection.PostProcessing.Generic.ICameraData
	{
		...
		*/
		private static float AdjustSize
		(
			float rawSize, float width, float height, float a, bool allowDR,
			float minPixels = 1.0f, float snapStep = 0.5f)
		{
			float refW = Config.referenceResolution.x;
			float refH = Config.referenceResolution.y;
			
			if(allowDR)
			{
				refW *= UnityEngine.ScalableBufferManager.widthScaleFactor;
				refH *= UnityEngine.ScalableBufferManager.heightScaleFactor;
			}
			
			float sW = width  / refW;
			float sH = height / refH;
			
			float scale = Mathf.Pow(sW, 1f - a) * Mathf.Pow(sH, a);
			
			float thicknessPx = rawSize * scale;
			
			thicknessPx = Mathf.Max(thicknessPx, minPixels);
		...
		#else
			private bool resolvedMsaaRequired { get { return false; } }
		#endif
